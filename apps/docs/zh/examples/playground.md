...
实时图算法演示。使用滑块调节节点数量，点击 **Run** 重新生成。滚轮缩放，拖拽平移。点击 **Show Code** 查看和编辑源代码。

## BFS — 广度优先遍历

逐层动画展示 Barabási–Albert 无标度网络的 BFS 探索过程。颜色越深代表 BFS 层级越深：

<Playground :code="animatedBFS" />

## 社区发现

基于模块度的标签传播算法识别植入分区网络中的密集连通簇。每种颜色代表一个检测到的社区：

<Playground :code="communityViz" />

## 介数中心性

Brandes 算法为每个节点计算通过它的最短路径数量。高分节点（暖色）是连接不同簇的结构性桥梁：

<Playground :code="centralityViz" />

## PageRank

有向网页图上的幂迭代 PageRank。节点大小编码权威分数 — 大的金色节点是"被链接最多"的枢纽：

<Playground :code="pageRankViz" />

## 性能基准测试

不同规模下的纯 JavaScript 计时。柱状图展示了 WASM 的必要性 — 500 节点的介数中心性在 JS 中已需数秒：
