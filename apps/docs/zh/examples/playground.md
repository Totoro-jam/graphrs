...
实时编辑代码查看效果。每个示例在数百个节点上运行**真实图算法**并带有动画可视化。使用滑块调整图规模，点击**重新生成**创建新的随机图。

## 网络遍历 — 无标度图上的 BFS

**真实场景**：模拟社交网络中的信息传播。Barabási–Albert 无标度图模拟了病毒式内容从枢纽账号向外传播的过程。BFS 揭示了分层的"六度分隔"结构 — 观察波浪如何从最高度数的枢纽节点向外扩散：

<Playground :code="animatedBFS" />

## 社区发现 — 社交网络聚类

**真实场景**：为推荐引擎识别用户群组。植入分区模型创建了内部连接密集、桥接稀疏的社区。基于模块度的标签传播检测聚类 — 这与社交平台上好友推荐背后的算法属于同一家族：

<Playground :code="communityViz" />

## 欺诈检测 — 介数中心性

**真实场景**：在交易网络中发现洗钱中间人。Brandes 算法识别出连接不同集群的网关账户（红色光晕）。动画粒子展示可疑资金沿高介数边流动的路径 — 这正是金融犯罪网络被检测的方式：

<Playground :code="centralityViz" />

## 网页权威度 — PageRank

**真实场景**：通过链接权威度对网页排名。有向图模拟了包含主题集群的网页爬取。PageRank（50 次迭代，含悬挂节点处理）识别出权威枢纽页面。金色粒子代表"链接汁液"在网络中流动 — 这是创建 Google 的算法：

<Playground :code="pageRankViz" />

## 性能 — 为什么需要 WASM

**瓶颈所在**：图算法是计算密集型的。纯 JavaScript 在中等规模下就会遇到瓶颈 — 500 个节点的介数中心性需要数秒，10000 个节点直接冻结 UI。此基准测试测量 JS 基线耗时，展示了为什么 `@graphrs` 使用 igraph 编译的 C 后端通过 WebAssembly 实现 **10–500 倍加速**：
